...
class SupabaseService {
  ...
  async createUser(user: Partial<DatabaseUser>) {
    ...
    const dbPayload: any = { ...user };
    
    // Combine names if full_name is missing
    if (!dbPayload.full_name && (user.first_name || user.last_name)) {
      dbPayload.full_name = `${user.first_name || ''} ${user.last_name || ''}`.trim();
    }
    
    // Remove legacy fields that don't exist in profiles
    delete dbPayload.first_name;
    delete dbPayload.last_name;
    delete dbPayload.organizationName;
    delete dbPayload.organizations;
