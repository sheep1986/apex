...
export class ErrorHandlingService {
  private static readonly ERROR_CODES = {
    ...
    // VAPI Errors
    VAPI_API_KEY_INVALID: {
      code: 'VAPI_API_KEY_INVALID',
      message: 'VAPI API key is invalid',
      userMessage: 'Your VAPI API key is invalid. Please check your API key settings.',
      severity: 'high' as const,
      category: 'api' as const,
      retryable: false,
    },
    VAPI_INSUFFICIENT_CREDITS: {
      code: 'VAPI_INSUFFICIENT_CREDITS',
      message: 'Insufficient VAPI credits',
      userMessage:
    ...
    VAPI_SYNC_FAILED: {
      code: 'VAPI_SYNC_FAILED',
      message: 'VAPI data sync failed',
      userMessage: 'Failed to sync data from VAPI. Some information may be outdated.',
      severity: 'medium' as const,
      category: 'api' as const,
      retryable: true,
    },
  ...
  private static parseError(error: any, context?: ErrorContext): ErrorDetails {
    ...
    // Handle VAPI errors
    else if (error?.message?.includes('VAPI') || error?.source === 'vapi') {
      errorDetails = this.parseVapiError(error, context);
    }
    // Handle Clerk errors
   ...
   * Parse VAPI errors
   */
  private static parseVapiError(error: any, context?: ErrorContext): ErrorDetails {
    const message = error.message || 'VAPI error';

    if (message.includes('API key') || message.includes('unauthorized')) {
      return { ...this.ERROR_CODES.VAPI_API_KEY_INVALID, context };
    }
    if (message.includes('credits') || message.includes('insufficient')) {
      return { ...this.ERROR_CODES.VAPI_INSUFFICIENT_CREDITS, context };
    }
    if (message.includes('call failed')) {
      return { ...this.ERROR_CODES.VAPI_CALL_FAILED, context };
    }
