import { createNotification, useNotificationStore } from '@/lib/notification-store';
...
class NotificationService {
  ...
  notifyVoiceSyncSuccess(data: { accountsCount: number; creditsTotal: number; callsTotal: number }) {
    this.addNotification(
      createNotification.success(
        'Voice Data Synced',
        `Successfully synced ${data.accountsCount} voice accounts. Total: ${data.creditsTotal.toLocaleString()} credits, ${data.callsTotal.toLocaleString()} calls`,
        {
          category: 'system',
          source: 'voice-sync',
          metadata: data,
  ...
  notifyVoiceSyncError(error: { message: string; accountId?: string; retryCount?: number }) {
    this.addNotification(
      createNotification.error(
        'Voice Sync Failed',
        `Failed to sync voice data: ${error.message}${error.retryCount ? ` (Retry ${error.retryCount})` : ''}`,
        {
          category: 'system',
          source: 'voice-sync',
          metadata: error,
  ...
  notifyVoiceAccountConnected(accountData: { name: string; credits: number; id: string }) {
    this.addNotification(
      createNotification.success(
        'Voice Account Connected',
        `Successfully connected voice account "${accountData.name}" with ${accountData.credits.toLocaleString()} credits`,
        {
          category: 'system',
  ...
  notifyVoiceAccountDisconnected(accountData: { name: string; reason?: string }) {
    this.addNotification(
      createNotification.warning(
        'Voice Account Disconnected',
        `Voice account "${accountData.name}" has been disconnected${accountData.reason ? `: ${accountData.reason}` : ''}`,
        {
          category: 'system',
          source: 'voice-integration',
          metadata: accountData,
  ...
  addDemoNotifications() {
    ...
    setTimeout(
      () => this.notifyVoiceSyncSuccess({ accountsCount: 2, creditsTotal: 12470, callsTotal: 1830 }),
      1000
    );
    setTimeout(
      () =>
