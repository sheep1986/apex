...
class OrganizationSettingsService {
  ...
  // Get Voice Engine credentials (with proper decryption for admins)
  async getVoiceCredentials(): Promise<VoiceCredentials> {
    const response = await apiClient.get(`${this.baseUrl}/voice/credentials`);
    return response.data;
  }

  // Convenience methods for common settings
  
  async setVoiceApiKey(apiKey: string): Promise<void> {
    await this.setSetting('voice_api_key', apiKey, true);
  }

  async setVoiceWebhookSecret(secret: string): Promise<void> {
    await this.setSetting('voice_webhook_secret', secret, true);
  }

  async setVoicePhoneNumbers(phoneNumbers: string[]): Promise<void> {
    await this.setSetting('voice_phone_numbers', phoneNumbers, false);
  }

  async setVoiceAssistants(assistants: any[]): Promise<void> {
    await this.setSetting('voice_assistants', assistants, false);
  }
    ...
    try {
      ...
      switch (capability) {
        case 'voice_integration':
          return !!settings.settings.voice_api_key;
        case 'webhooks':
          return !!settings.settings.voice_webhook_secret;
        case 'phone_numbers':
          return Array.isArray(settings.settings.voice_phone_numbers) && 
                 settings.settings.voice_phone_numbers.length > 0;
        case 'assistants':
          return Array.isArray(settings.settings.voice_assistants) && 
                 settings.settings.voice_assistants.length > 0;
        default:
          return false;
      }
    } catch (error) {
  ...
  async getCampaignDefaults(): Promise<{
    ...
    return {
      ...
      voiceCredentials: voiceCreds
    };
  }

  // Test Voice Engine connection
  async testVoiceConnection(): Promise<{ success: boolean; message: string; details?: any }> {
    try {
      const response = await apiClient.post('/voice/test-connection');
      return response.data;
    } catch (error: any) {
      return {
        success: false,
  ...
  // Sync Voice resources (phone numbers, assistants)
  async syncVoiceResources(): Promise<{
    phoneNumbers: string[];
    assistants: any[];
    success: boolean;
  }> {
    try {
      const response = await apiClient.post('/voice/sync-resources');
      
      // Update local settings with synced data
      if (response.data.phoneNumbers) {
        await this.setVoicePhoneNumbers(response.data.phoneNumbers);
      }
      
      if (response.data.assistants) {
        await this.setVoiceAssistants(response.data.assistants);
      }
      
      return response.data;
    } catch (error) {
      console.error('Error syncing Voice resources:', error);
      throw error;
    }
  }
