...
import { safeApi, handleApiResult } from '@/lib/safeApi';
...
class CallsService {
  ...
  async getCallDetails(callId: string): Promise<CallDetails> {
    ...
    const result = await safeApi<{ call: CallDetails }>({
      method: 'GET',
      url: `${API_BASE_URL}/api/vapi-outbound/calls/${callId}`
    });

    if (!result.ok) {
      throw new Error(result.error.message || 'Failed to fetch call details');
    }
  ...
  async getCampaignCalls(campaignId: string): Promise<{ calls: CallSummary[] }> {
    ...
    const result = await safeApi<{ calls: CallSummary[] }>({
      method: 'GET',
      url: `${API_BASE_URL}/api/vapi-outbound/campaigns/${campaignId}/calls`
    });

    if (!result.ok) {
      throw new Error(result.error.message || 'Failed to fetch campaign calls');
    }
