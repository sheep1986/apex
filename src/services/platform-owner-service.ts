...
class PlatformOwnerService {
    ...
    return [
      {
        ...
        issues: [
          {
            type: 'api-error',
            message: 'VAPI API key expired or invalid',
            severity: 'high',
          },
        ],
        metrics: {
  ...
  async generateAIResponse(userMessage: string): Promise<string> {
    ...
    if (lowerMessage.includes('vapi') || lowerMessage.includes('api')) {
      return "I see you're having an issue with VAPI integration. Let me check your API key status and run some diagnostics. This looks like it might need technical assistance.";
    }
