...
const VoiceDashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const safeLoadData = async () => {
      try {
        ...
        console.error('🚨 VoiceDashboard: Critical error during initialization:', error);
  ...
  const loadDashboardData = async (showLoader = true) => {
    ...
    try {
      ...
      // Cast to any for calculation purposes if types mismatch slightly
      const analyticsData = (campaignsData as any[]).reduce((acc, campaign) => {
        return {
  ...
  const handleCampaignAction = async (campaignId: string, action: 'start' | 'pause' | 'resume') => {
    try {
      ...
      switch (action) {
        case 'start':
          result = await campaignOutboundService.startCampaign(campaignId);
          break;
        case 'pause':
          result = await campaignOutboundService.pauseCampaign(campaignId);
          break;
        case 'resume':
          result = await campaignOutboundService.resumeCampaign(campaignId);
          break;
        default:
  ...
  return (
    <div className="w-full bg-black min-h-screen px-4 sm:px-6 lg:px-8 py-6">
                    ...
                    <div className="relative z-10">
                      <div className="mb-6 flex items-start justify-between">
                        <div className="flex-1">
                          ...
                          <p className="text-sm text-gray-400 font-medium">ID: {campaign.apexId || 'provider00000'}</p>
                        </div>
                        <div className="flex items-center space-x-3">
      ...
      {showEditModal && selectedCampaign && (
        <CampaignEditWizard
          campaign={selectedCampaign as any} // Fixing type mismatch temporarily
