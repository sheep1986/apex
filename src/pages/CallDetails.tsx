...
export default function CallDetailsPage() {
  ...
  const handlePlayPause = async () => {
    if (audioRef.current) {
      try {
        ...
        // If recording URL fails, try opening in new tab
        if (call?.recording) {
    ...
    try {
      ...
      const existingLead = existingLeads.find(
        (lead) =>
          ...
          lead.customFields?.providerCallId === callData.providerCallId
      );
  ...
  const handleSyncCall = async () => {
    ...
    try {
      setLoading(true);
      const response = await callsService.syncCall(call.providerCallId);
      
      if (response.success) {
        toast({
          title: 'Call Synced',
          description: 'Call data has been updated from provider',
        });
      ...
      toast({
        title: 'Sync Failed',
        description: 'Failed to sync call data from provider',
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
      ...
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-3">
            ...
            {(call.status === 'answered' || call.status === 'ringing' || !call.transcript) && (
              <Button
                onClick={handleSyncCall}
                size="sm"
                variant="outline"
                disabled={loading || !call.providerCallId}
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          ...
          <Card className="border-gray-700 bg-gray-800/50">
            ...
            <CardContent className="space-y-3">
              ...
              <div className="flex items-center justify-between">
                <span className="text-gray-400">Provider Call ID:</span>
                <div className="flex items-center space-x-2">
                  <span className="font-mono text-xs text-white">
                    {call.providerCallId ? `${call.providerCallId.substring(0, 8)}...` : 'N/A'}
                  </span>
                  <Button
                    size="sm"
                    variant="ghost"
                    onClick={() => call.providerCallId && handleCopyToClipboard(call.providerCallId, 'Provider Call ID')}
                    disabled={!call.providerCallId}
                  >
                    <Copy className="h-3 w-3" />
                  </Button>
                </div>
