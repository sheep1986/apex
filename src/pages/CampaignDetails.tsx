...
const defaultCampaign = {
  ...
  settings: {
    ...
    // Add these for TS inference
    assistant_id: '',
    system_prompt: '',
  },
  createdAt: new Date().toISOString(),
  lastModified: new Date().toISOString(),
  // Add missing fields for type inference
  assistantId: '',
  systemPrompt: '',
  totalLeads: 0,
  callsCompleted: 0,
...
export default function CampaignDetails() {
  ...
  // const vapiOutboundService = useVapiOutboundService(); // REMOVED
  const [isEditing, setIsEditing] = useState(false);
  const [campaign, setCampaign] = useState(defaultCampaign);
  const [originalCampaign, setOriginalCampaign] = useState(defaultCampaign);
    ...
    if (call.id) {
      try {
        ...
        const transformedCallData = {
          ...
          providerCallId: fullCallData.vapiCallId || fullCallData.vapi_call_id,
  ...
  /* const mockCalls = [
    ...
    },
    {
      ...
      status: 'completed',
      recording: 'https://storage.trinity-ai.com/recording-002.wav',
      transcript:
    ...
    },
    {
      ...
      status: 'completed',
      recording: 'https://storage.trinity-ai.com/recording-003.wav',
      transcript:
    ...
    },
    {
      ...
      status: 'completed',
      recording: 'https://storage.trinity-ai.com/recording-005.wav',
  ...
  const handleUpdateVapiPrompt = async () => {
    ...
    if (!assistantId) {
      toast({
        title: 'Error',
        description: 'No Voice Engine assistant ID found for this campaign',
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      ...
      // Update Voice assistant first
      // Fetch current assistant to preserve model settings
      await voiceService.updateAssistant(
        assistantId,
        {
          model: {
            systemMessage: campaign.systemPrompt
          }
        } as any
      );
      ...
      toast({
        title: 'Success',
        description: 'System prompt updated in both Voice Engine and database successfully',
      });
      
      // Update the original campaign state so cancel works properly
      setOriginalCampaign({ ...campaign });
    } catch (error) {
      console.error('Error updating Voice Engine prompt:', error);
      toast({
        title: 'Error',
        description: 'Failed to update system prompt in Voice Engine',
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
          ...
          <TabsContent value="script" className="mt-6">
            <Card className="border-gray-800 bg-gray-900">
              <CardHeader>
                ...
                <CardDescription className="text-gray-400">
                  The system prompt used by the voice assistant for this campaign
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                          ...
                          <Button
                            onClick={async () => {
                              ...
                              if (assistantId) {
                                setLoading(true);
                                try {
                                  const details = await voiceService.getAssistant(assistantId);
                                  if (details?.model?.systemMessage) {
                                    updateCampaign('systemPrompt', details.model.systemMessage);
                                    toast({
                                      title: 'Success',
                                      description: 'Fetched latest prompt from Voice Provider',
                                    });
                                  }
                                } catch (err) {
                                  toast({
