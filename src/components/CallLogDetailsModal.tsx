  ...
  */
    ...
    if (audioRef.current && callData && callData.recording) {
      try {
        if (isPlaying) {
          ...
          // Use direct URL - Voice Engine recordings should have proper CORS headers
          // If this fails, user can download the recording instead
        ...
        // Show error message
        alert('Unable to play audio directly due to browser restrictions. Please use the Download button to save the recording.');
        setIsPlaying(false);
      }
  ...
  const handleTranscriptClick = (index: number) => {
    if (callData) {
      ...
      if (typeof item !== 'string' && item.startTime !== undefined && audioRef.current) {
      ...
      >
        <div className="flex h-full flex-col">
          ...
          <div className="overflow-y-auto flex-shrink-0 border-b border-gray-700/50 bg-[#1a1a1a] p-4" style={{ maxHeight: '40%' }}>
            ...
            {/* Call MetacallData */}
            {(callData.campaignName || callData.startedAt || callData.providerCallId) && (
                ...
                {callData.providerCallId && (
                  <div className="flex items-center space-x-2 text-sm">
                    ...
                    <span className="font-mono text-gray-300">
                      {callData.providerCallId.substring(0, 8)}...
                    </span>
                  </div>
                )}
              </div>
                      ...
                      <button 
                        onClick={() => {
                          if (callData.recording) {
                            // Open recording in new tab for download
                            // This avoids CORS issues and lets user download directly
