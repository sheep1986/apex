...
export const CampaignEditWizard: React.FC<CampaignEditWizardProps> = ({ 
  campaign,
  onCampaignUpdated, 
  onCancel 
}) => {
  ...
  const [campaignType, setCampaignType] = useState(campaign.campaignType === 'b2b' ? 'b2b' : campaign.campaignType === 'b2c' ? 'b2c' : 'manual');
  const [phoneNumber, setPhoneNumber] = useState(campaign.phoneNumber || '');
  const [assistant, setAssistant] = useState(campaign.assistantName || '');
...
**⚙️ API Optimization**
- 🚫 Prevents hitting the Voice Engine API rate limits which could pause your campaign
...
**🔌 Voice Engine API Limits**
- 🏢 The platform may have account-specific concurrency limits
...
- 🔍 Check Voice Engine account limits
...
+1-234-423-4567,Bob Johnson,Test`;
  ...
  const handleUpdateCampaign = async () => {
    ...
    try {
      ...
      const campaign = await campaignOutboundService.createCampaign(campaignData);

      // Upload leads
      if (csvFile) {
        await campaignOutboundService.uploadLeads(campaign.id, csvFile);
      }
      ...
      // Update the campaign instead of creating a new one
      const updatedCampaign = {
        ...campaign,
        name: campaignName,
        assistantName: assistant,
        phoneNumber: phoneNumber,
        campaignType: campaignType as 'b2b' | 'b2c',
        settings: {
          ...campaign.settings,
          callsPerHour: enableRateLimiting ? callsPerHour : 60,
          callsPerMinute: enableRateLimiting ? callsPerMinute : 10,
        ...
        <div className="h-16 bg-gray-900 border-b border-gray-800 flex items-center justify-between px-6">
          <div className="flex items-center space-x-3">
            ...
            <h1 className="text-lg font-semibold text-white">
              Edit campaign {currentStep === 2 && '- Rate Limiting'}{currentStep === 3 && '- Retry Logic'}{currentStep === 4 && '- Working Hours'}{currentStep === 5 && '- Team Management'}{currentStep === 6 && '- Review & Save'}
            </h1>
          </div>
          <div></div>
        </div>
          ...
          <div className={`${currentStep === 1 ? 'w-1/2' : showHelpPanel ? 'w-1/2' : 'w-full'} flex flex-col transition-all duration-300`}>
            ...
            <div className="p-6 border-t border-gray-800 bg-gray-900">
              <div className="flex items-center justify-between">
                ...
                <Button 
                  onClick={currentStep === 6 ? handleUpdateCampaign : handleNext}
                  disabled={isLoading}
                  className="bg-emerald-600 hover:bg-emerald-700 text-white"
                >
                  {currentStep === 6 ? (isLoading ? 'Saving...' : 'Save Changes') : 'Next'}
                </Button>
              </div>
            </div>
          </div>
