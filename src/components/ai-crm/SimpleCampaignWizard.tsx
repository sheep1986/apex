...
import { AlertCircle, AlertTriangle, BarChart3, Calendar, CheckCircle, ChevronLeft, Clock, DollarSign, ExternalLink, Globe, Info, Settings, Sparkles, Target, Upload, Users, Wallet, X, Zap, Zap as ZapIcon } from 'lucide-react';
...
import { CampaignTeamManager, TeamInvitation, TeamMember } from './CampaignTeamManager';
...
export const SimpleCampaignWizard: React.FC<SimpleCampaignWizardProps> = ({ 
  ...
  const [concurrentCalls, setConcurrentCalls] = useState(10); // default from System
  ...
  // Load Voice Engine assistants and phone numbers using organization's credentials
  const loadApexData = async () => {
    try {
      ...
      setPhoneNumbers(phoneNumbersData as any[]);
      
      // Show success message if we got data
      if (assistantsData.length > 0 || phoneNumbersData.length > 0) {
        toast({
          title: 'Voice Engine Connected',
          description: `Loaded ${assistantsData.length} assistants and ${phoneNumbersData.length} phone numbers`,
          variant: 'default'
        });
      } else {
        toast({
          title: 'No Voice Resources',
          description: 'Please configure assistants and phone numbers in your dashboard',
          variant: 'default'
        });
      }
    } catch (err: any) {
      console.error('❌ Failed to load Voice Engine data:', err);
      ...
      toast({
        title: 'Voice Configuration Required',
        description: 'Your organization admin needs to configure Voice Engine settings',
        variant: 'destructive'
      });
    } finally {
...
**⚙️ API Optimization**
- 🚫 Prevents hitting Platform API rate limits which could pause your campaign
...
**🔌 Platform Limits**
- 🏢 Platform may have account-specific concurrency limits
...
**💵 Cost Breakdown:**
- Voice Engine hosting fees ($0.05/min)
...
- 🔄 **Concurrent Calls:** Based on platform plan (10 default)
...
- 🚨 Set up balance alerts in dashboard
      `
    }
  };
                ...
                <SelectContent className="bg-gray-800 border-gray-700">
                  {assistants.length === 0 ? (
                    <SelectItem value="none" disabled>
                      {loadingApexData ? "Loading..." : "No assistants available - Check Voice Engine configuration"}
                    </SelectItem>
                  ) : (
                    assistants.map((asst) => (
                          ...
                          <div className="flex items-center justify-between">
                            <div className="flex items-center space-x-3">
                              ...
                              <div>
                                ...
                                <p className="text-xs text-gray-400 mt-1">
                                  Filter out contacts before sending to Voice Engine based on their local time
                                </p>
                              </div>
                            </div>
                            <Switch
                  ...
                  <div className="bg-gray-900/50 rounded-lg p-4 mb-4">
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between">
                        <span className="text-gray-400">Voice Engine Hosting</span>
                  ...
                  <div className="flex items-start space-x-3">
                    ...
                    <div className="text-sm text-gray-400">
                      ...
                      <ul className="space-y-1 list-disc list-inside">
                        ...
                        <li>Consider upgrading your platform plan for better per-minute rates on high-volume campaigns</li>
