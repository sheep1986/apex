...
export const AccountSetup: React.FC<AccountSetupProps> = ({ 
  ...
  const steps = [
    { title: 'Trinity Configuration', description: 'Connect your Trinity account' },
  ...
  const validateStep = (step: number): boolean => {
    ...
    switch (step) {
      case 0:
        // Automatically valid as it's managed
        break;
      case 1:
  ...
  const handleNext = () => {
    if (validateStep(currentStep)) {
      if (currentStep === 0) {
        // Mock successful retrieval of managed resources
        const mockResult = {
          isValid: true,
          assistants: [
            { id: '1', name: 'Sales Assistant', model: 'gpt-4', voice: 'alloy' },
            { id: '2', name: 'Support Assistant', model: 'gpt-4', voice: 'echo' },
            { id: '3', name: 'Customer Service Assistant', model: 'gpt-3.5-turbo', voice: 'shimmer' }
          ],
          phoneNumbers: [
            { id: '1', number: '+1 (555) 123-4567', provider: 'twilio', active: true },
            { id: '2', number: '+1 (555) 987-6543', provider: 'twilio', active: true },
            { id: '3', number: '+1 (555) 456-7890', provider: 'twilio', active: true }
          ]
        };
        ...
        <CardContent className="space-y-6">
          ...
          {currentStep === 0 && (
            <div className="space-y-4">
              <div className="bg-gray-900/50 border border-gray-800/50 p-4 rounded-lg">
                <h3 className="font-medium mb-2 text-white">Trinity Voice Engine</h3>
                <p className="text-sm text-gray-400 mb-3">
                  Your organization's voice engine is managed centrally by Trinity Labs AI.
                </p>
                <div className="flex items-center p-3 mb-2 text-emerald-400 bg-emerald-400/10 rounded-lg border border-emerald-400/20">
                  <CheckCircle className="w-5 h-5 mr-2" />
                  <span className="font-medium">Voice Engine Active: Managed Mode</span>
                </div>
                 <p className="text-xs text-gray-500">
                  No API key configuration required. System is ready.
                </p>
              </div>
            </div>
          )}
          ...
          {currentStep === 3 && (
            <div className="space-y-4">
              ...
              {validationResult && (
                <div className="space-y-2">
                  <h4 className="font-medium">Trinity Resources</h4>
                  <p className="text-sm">
