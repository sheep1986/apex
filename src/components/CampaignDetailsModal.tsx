...
const CampaignDetailsModal: React.FC<CampaignDetailsModalProps> = ({
  ...
  const loadCampaignData = async () => {
    try {
      ...
      const [dashboardData, resultsData] = await Promise.all([
        vapiOutboundService.getCampaignDashboard(campaign.id),
        vapiOutboundService.getCampaignResults(campaign.id),
      ]);
  ...
  const loadLiveData = async () => {
    try {
      const data = await vapiOutboundService.getLiveData(campaign.id);
      setLiveData(data);
    } catch (error) {
    ...
    try {
      ...
      switch (action) {
        case 'start':
          ...
          break;
        case 'pause':
          await vapiOutboundService.pauseCampaign(campaign.id);
          break;
        case 'resume':
          await vapiOutboundService.resumeCampaign(campaign.id);
          break;
      }
