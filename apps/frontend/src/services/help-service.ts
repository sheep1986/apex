...
Welcome to Apex AI! This guide will help you make your first AI call in under 5 minutes.
...
### Model Settings
- **GPT-4**: Most intelligent (recommended)
...
- **Interruption Handling**: How AI responds to interruptions
...
- Verify your microphone settings
...
When you complete onboarding, Apex automatically creates:
...
Need help? Our support team can assist with Airtable setup and optimization.
  ...
  async getFAQItems(category?: string): Promise<FAQItem[]> {
    const faqs: FAQItem[] = [
      ...
      },
      {
        ...
        answer:
          'Our AI voices are powered by ElevenLabs, the most advanced voice synthesis technology available. Most people cannot distinguish them from human voices. We offer multiple voice options to match your brand and audience.',
        category: 'ai-assistants',
        helpful: 38,
        order: 2,
      },
  ...
  async getQuickHelp(context: {
    ...
    const suggestions: { [key: string]: string[] } = {
      onboarding: ['quick-start-guide', 'vapi-integration-guide'],
...
export type { HelpArticle, HelpCategory, VideoTutorial, FAQItem, SupportTicket };
