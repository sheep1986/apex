...
export default function OrganizationManagement() {
  ...
  const [vapiWebhookUrl, setVapiWebhookUrl] = useState('');
  ...
  const fetchOrganizationData = async () => {
    try {
      ...
      if (org.vapi_settings || org.settings?.vapi) {
        ...
        setVapiApiKey(vapiSettings.apiKey || '');
  ...
  const handleSaveChanges = async () => {
    try {
      ...
      if (!orgResponse.ok) throw new Error('Failed to update organization');

      // Update VAPI settings if on settings step
      if (currentStep === 3) {
        const settingsResponse = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3001'}/api/organizations/${id}/settings`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
            ...(token && { Authorization: `Bearer ${token}` }),
        ...
        if (!settingsResponse.ok) throw new Error('Failed to update VAPI settings');
      }
            ...
            {currentStep === 3 && (
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  <div>
                    <Label htmlFor="vapiApiKey" className="text-gray-300">
                      VAPI API Key
                    </Label>
