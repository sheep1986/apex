...
export default function CRM() {
  ...
  const getMockCallData = (contact: Contact) => {
    return {
      ...
      transcript: [
        { speaker: 'ai' as const, text: `Hello, is this ${contact.firstName}?` },
        { speaker: 'user' as const, text: 'Yes, this is me.' },
        {
          speaker: 'ai' as const,
          text: `Hi ${contact.firstName}, this is Sarah from Apex AI Solutions. I hope I'm not catching you at a bad time. I'm calling because we've developed an innovative AI calling solution that's been helping companies like ${contact.company} increase their outreach efficiency. Would you be interested in learning more?`,
        },
        {
      ...
      analysis: {
        ...
        keywords: ['AI', 'solution', contact.company, 'efficiency'],
        summary: `Call with ${contact.firstName} ${contact.lastName} from ${contact.company}. ${contact.status === 'interested' ? 'Expressed strong interest in AI solutions.' : contact.status === 'contacted' ? 'Showed moderate interest, needs follow-up.' : 'Limited interest shown.'}`,
      },
  ...
  const renderCellContent = (columnKey: string, contact: Contact) => {
    ...
    switch (columnKey) {
      ...
      case 'status':
        return (
          <Badge className={`${statusConfig[contact.status].color} px-2 py-1 text-xs font-medium`}>
      ...
      case 'phone':
        return (
          <div className="flex items-center">
            ...
            <span className="text-xs text-white font-medium">{contact.phone}</span>
          </div>
        );
      case 'cid':
                        ...
                        <CardContent className="p-4">
                          <div className="mb-3 flex items-start justify-between">
                            <div className="flex-1">
                              ...
                              <div className="mt-1 text-xs text-gray-400">{contact.email}</div>
                            </div>
                          ...
                          <div className="mb-3 flex items-center justify-between">
                            <Badge
                              className={`${statusConfig[contact.status].color} text-xs text-white`}
                            >
                              <StatusIcon className="mr-1 h-3 w-3" />
                              {statusConfig[contact.status].label}
                            </Badge>
