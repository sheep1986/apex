...
export class CampaignExecutor {
  ...
  /**
   * Parse CSV data into contacts array
   */
  ...
  /**
   * Transform campaign data from database to expected format
   */
  private transformCampaignData(rawCampaign: any): CampaignSettings {
    // If campaign uses new settings structure
    ...
    return rawCampaign as CampaignSettings;
  }
  ...
  private async startCampaign(campaignId: string) {
    ...
    // Get campaign to check for CSV data in settings
    ...
    // First try to get contacts from campaign_contacts table (old way)
  ...
  private async scheduleRetryIfNeeded(queuedCall: QueuedCall, outcome: string) {
    ...
    // Check both old retry_settings field and new settings.retrySettings
