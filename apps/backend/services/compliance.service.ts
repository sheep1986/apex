...
// Note: libphonenumber-js dependency not installed - using basic phone validation
...
export class ComplianceService {
  ...
  private inferTimezone(phoneNumber: string): string {
    try {
      // Simple phone number country detection
  ...
  private getStateFromPhoneNumber(phoneNumber: string): string {
    try {
      // Simple US phone number parsing
      const cleanPhone = phoneNumber.replace(/\D/g, '');
      ...
      const areaCode = cleanPhone.substr(1, 3); // Skip country code 1
