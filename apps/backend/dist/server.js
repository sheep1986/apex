...
const vapi_webhook_1 = __importDefault(require("./api/vapi-webhook"));
...
const vapi_sync_1 = __importDefault(require("./api/vapi-sync"));
...
app.use('/api/vapi-enhanced/webhook', raw_body_1.rawBodyMiddleware);
app.use('/api/vapi/webhook', raw_body_1.rawBodyMiddleware);
app.use('/api/vapi-automation-webhook', raw_body_1.rawBodyMiddleware);
app.use('/api/stable-vapi/webhook', raw_body_1.rawBodyMiddleware);
...
app.use('/api/vapi-sync', clerk_auth_1.authenticateUser, vapi_sync_1.default);
...
app.use('/api/platform-monitoring', clerk_auth_1.authenticateUser, platform_monitoring_1.default);
...
app.post('/api/vapi-auto-setup', clerk_auth_1.authenticateUser, vapi_auto_setup_1.autoConfigureVAPI);
app.use('/api/vapi-enhanced', vapi_webhook_enhanced_1.default);
app.use('/api/vapi', vapi_webhook_1.default);
...
app.listen(PORT, () => {
    ...
    console.log('🎯 Starting campaign automation system (with settings column support)...');
    campaign_executor_1.campaignExecutor.start();
    console.log('✅ Campaign executor started at:', new Date().toISOString());
