    ...
    try {
        ...
        const campaignsWithMetrics = await Promise.all((campaigns || []).map(async (campaign) => {
            try {
                ...
                const completedCalls = calls?.filter(c => c.status === 'completed' || c.outcome === 'completed' || c.duration > 0).length || 0;
                const totalCost = calls?.reduce((sum, call) => {
                    if (call.cost)
                        return sum + call.cost;
                    if (call.duration)
                        return sum + (call.duration / 60 * 0.15);
                    return sum;
                }, 0) || 0;
                return {
                    id: campaign.id,
...
router.get('/assistants', async (req, res) => {
    try {
        ...
        let apiError = null;
        try {
            assistants = await vapiService.getAssistants();
