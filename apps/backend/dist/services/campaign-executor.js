...
class CampaignExecutor {
    ...
    async processCampaign(campaign) {
        ...
        if (campaign.status === 'active') {
            const { count: queueCount } = await supabase_client_1.default
                .from('call_queue')
    ...
    async startCampaign(campaignId) {
        ...
        if (!error && existingContacts && existingContacts.length > 0) {
            ...
            if (parsedContacts.length > 0) {
                ...
                const { data: createdContacts, error: createError } = await supabase_client_1.default
                    .from('campaign_contacts')
    ...
    async saveCallRecord(queuedCall, vapiResult, outcome) {
        ...
        const callRecord = {
            ...
            started_at: vapiResult.call?.startedAt || queuedCall.lastAttemptAt,
