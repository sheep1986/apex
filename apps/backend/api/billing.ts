...
if (stripeSecretKey && stripeSecretKey !== 'dummy') {
  try {
    stripe = new Stripe(stripeSecretKey, {
      apiVersion: '2025-08-27.basil' as any, // Temporarily bypass version check
    });
...
router.post('/cancel-subscription', authenticateUser, async (req: any, res: Response) => {
  try {
    ...
    const canceledSubscription = await stripe?.subscriptions.cancel(userSub.stripe_subscription_id);
